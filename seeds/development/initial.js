    ...
    knex('wedding_profiles').count('id')
        .then(([{ count: countString }]) => {
            ...
            return knex.transaction(trx =>
                knex('sections').transacting(trx).del()
                    .then(() => knex('gift_sets_gifts').transacting(trx).del())
                    .then(() => knex('gifts').transacting(trx).del())
                    .then(() => knex('wedding_party_members').transacting(trx).del())
                    .then(() => knex('wedding_profiles').transacting(trx).del())
                    .then(() => knex('wedding_profiles').transacting(trx).insert({
                        id: 1,
